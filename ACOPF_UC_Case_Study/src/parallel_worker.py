...
def _worker_fedavg(args):
    ...
    for sample_np in samples_np:
        ...
        try:
            ...
            # --- NN Training Loss (NO gen_cost) ---
            # gen_cost must NOT appear here.  The CVXPY already minimises cost
            # internally for each specific load input — that is the correct place
            # for cost minimisation.  Putting gen_cost in the NN loss makes the
            # NN collapse: it discovers that [0,1,1,1,0] is the cheapest
            # configuration for ALL loads and predicts it universally (mode
            # collapse).  Removing it forces the gradient signal to come only
            # from integrality_pen and slack_pen, which ARE load-specific, so
            # the NN learns different predictions for different load profiles.
            integrality_pen = torch.sum(u_relax * (1.0 - u_relax))
            slack_pen       = torch.sum(s_cut) + torch.sum(xi_c) + torch.sum(xij_c) + torch.sum(xij_s)

            loss = (lambda_integrality * integrality_pen
