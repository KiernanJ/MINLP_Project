...
def build_diffopt_qcac_layer(data, num_cuts=10):
    ...
    # Minimum generation constraint: at least 1 (fractional) generator must
    # be on.  Without this, the CVXPY finds the trivial global optimum u=0:
    #   cost(u=0) = 0, while QCAC slacks absorb the power-balance mismatch.
    # For case14 the slack cost is cheaper than activating generators, so the
    # unconstrained CVXPY always drives u -> 0.  This single constraint
    # eliminates that degenerate solution without changing the optimal integer
    # solution (any physically feasible UC always has at least one gen on).
    constraints.append(cp.sum(u_relax) >= 1.0)
    # -----------------------------------------------------------------
    ...
    # Minimize generation cost PLUS the dynamically learned slack penalty PLUS fixed cut penalty
    obj = cp.Minimize(cost_expr + rho * slack_sum + 100.0 * cp.sum(s_cut))
